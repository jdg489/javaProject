...
public class ListRun {
	...
	/*
	 	컬랙션이란
	 	자료구조 개념이 내장되어있는 클래스로 자바에서 제공하는 "지료구조"를 담당하는 "프레임워크"이다.
	 	...
	 	배열의 단점
	 	1. 배열은 크기를 지정해야 한다.
	 		새로운 값을 더 추가하고자 한다면 새로운 크기의 배열을 만들고 기존의 값을 전부 이동시켜줘야한다.
	 		
	 	2. 배열 중간에 값을 추가하거나 삭제하는 경우 값을 매번 이동시켜주는 복잡한 코드를 직접 작성해야한다.
	 		 	
	 	3. 한 공간에 한 타입이 데이터들만 저장이 가능하다.
	 	
	 	컬랙션의 장점
	 	1. 크기를 지정해 줄 필요가 없다.
	 		만일 지정했다고해도 더 많은 데이터들을 담으면 알아서 사이즈를 늘려준다.
	...
	public static void main(String[] args) {
		// ArrayList list
		List list = new ArrayList(3); // 크기 지정을 할수도 있고 안해도된다.
		...
		// 4. set(int index E e) : 해당인덱스 위치의 값을 전달받은 e 객체로 변경
		...
		// 7. subList(int index1, int index2) : List -> 추출해서 새로운 List 변환
		List sub = list.subList(0, 2); // 1번부터 3번 인덱스 전까지
		System.out.println(sub);
		
		// 8. addAll(Collection c) : 컬랙션을 통째로 뒤에 추가시켜주는 메소드
		list.addAll(sub);
		System.out.println(list);
		
		// 9. isEmpty() : boolean -> 컬랙션이 비어있는지 묻는 메소드
