...
//		}
		...
		// 학생성적관리 프로그램을 만들고자한다.
		// 학생 클래스를 정의해보자. Student class 는 name(String), classRoom(int),
	...
	for(int i =0; i < stArr.length; i++) {
		...
		System.out.printf("%5s %3s%4s %4s", "이름", "반","자바","sql","실습");
		for(int i = 0; i<stArr.length; i++) {
			Student st = stArr[i];
			System.out.printf("%5s %3d %4.2f %4.2f\n", st.getName(), st.getClassRoom(), st.getJavaScore(), st.getPracticeScore(), st.getSqlScore());
		}
	}
}
